...
const NavBar: React.FC = () => {
  ...
  console.log("NavBar userName:", userName, "userPicture:", userPicture);
  ...
  return (
    <>
      ...
      >
        {isMobile && (
          <div className="flex items-center">
            <button
              ...
              className="transition-all cursor-pointer group hover:scale-[1.1]"
            >
        ...
        <div
          ref={searchRef}
          className="relative w-full flex items-center max-w-lg"
        >
          <input
            type="text"
            placeholder={t("search") + "..."}
            ref={inputRef}
            className={`bg-[#171719] p-2 px-5 pl-10 rounded-full transition-all ${
              isInputOpen
          ...
          />
          <button
            onClick={() => isMobile && setIsInputOpen((val) => !val)}
            className={`absolute left-1 rounded-full ${
              isMobile ? "cursor-pointer" : ""
            } ${
              isMobile && !isInputOpen ? "p-4" : ""
            } ${isInputOpen ? "bg-[#1e1e22] p-2" : "bg-[#171719]"}`}
          >
            <Search className="w-4 h-4" />
          </button>
        </div>

        {/* Основной блок с кнопками или аватаром */}
        {!isMobile ? (
          <div className="flex items-center gap-2">
            {userName ? (
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-12 bg-[#2a2a2e] border border-gray-600 rounded-lg shadow-lg py-2 min-w-[150px] z-50">
                    ...
                    <button
                      ...
                      className="w-full px-4 py-2 text-left text-sm text-white hover:bg-[#3a3a3e] transition-all flex items-center gap-2"
                    >
              ...
              <>
                <Link
                  to="/sign_up"
                  className="text-sm font-bold transition-all hover:text-gray-400 hover:scale-[1.04] active:scale-[0.95] p-1 tracking-wide text-nowrap cursor-pointer"
                >
                  {t("sign_up")}
                </Link>
                <Link
                  to="/sign_in"
                  className="bg-white text-black rounded-full font-bold text-nowrap transition-all hover:scale-[1.04] active:scale-[0.95] p-2 px-5 cursor-pointer"
                >
                  {t("sign_in")}
                </Link>
              </>
      ...
      <div
        ref={modalRef}
        className={`fixed inset-0 bg-[#212124] z-9999 p-4 transform transition-all
        ${
          isModalOpen
